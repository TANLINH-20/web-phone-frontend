...
class ShoppingCart extends Component {
  ...
  getGuestShoppingCartList(localCartList) {
    ...
    axios
      ...
      .then((response) => {
        ...
        this.props.updateCartCount(JSON.parse(localCartList));
      });
  }
  ...
  handleChange(e) {
    if (e.target.type === "checkbox") {
      ...
      axios
        .put(`http://127.0.0.1:8000/api/product/cart-list/${e.target.id}`, {
          quantity: quantity,
        })
        .then((response) => {
  ...
  handleDelete(e) {
    ...
    if (this.state.userId) {
      axios
        .delete(`http://127.0.0.1:8000/api/products/cart-list/${id}`, {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        })
        .then((response) => {
          if (response.status === 200) {
      ...
      this.props.updateCartCount(JSON.parse(localStorage.getItem("cartList")));
                      ...
                      <div className="media cart-item-box">
                        ...
                        <div className="media-body cart-item-body">
                          <h5 className="mt-0 product-name">
                            <Link to={`/product/${item.stock.product.id}`}>
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    deleteCartItem: (id) => dispatch({ type: "DELETE_CART_ITEM", payload: id }),
  };
};
